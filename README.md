...
3. ディレクトリ構造
Project_Root/
├── data/               # 原始画像（.tif など）の格納
├── scripts/
│   ├── slice_image.py  # 画像の切片化スクリプト
│   ├── auto_label.py   # OpenCVによる自動アノテーション
│   ├── train.py        # YOLOv8 学習用スクリプト
│   └── detect_full.py  # 大画像のスライディングウィンドウ推論
├── models/
│   └── best.pt         # 学習済みモデル（Git LFS管理）
